(function umd(root,factory){
	...
	Component.prototype = {
		...
		load: function(componentURL) {
			...
			return VueLoader.httpRequest(componentURL)
		...
		_normalizeSection: function(eltCx) {
			...
			if ( eltCx === null || !eltCx.elt.hasAttribute('src') ) {
				...
				p = VueLoader.httpRequest(eltCx.elt.getAttribute('src'))
			...
			.then(function(content) {
				...
				if ( eltCx !== null && eltCx.elt.hasAttribute('lang') ) {
					...
					return VueLoader.langProcessor[lang.toLowerCase()].call(this, content === null ? eltCx.getContent() : content);
				}
				return content;
			}.bind(this))
			.then(function(content) {
				...
				for ( var componentName in components ) {
					...
					if ( typeof(components[componentName]) === 'string' && components[componentName].substr(0, 4) === 'url:' ) {
						...
						if ( isNaN(componentName) )
							components[componentName] = VueLoader.load(componentURL, componentName);
						else
							components[componentName] = Vue.component(comp.name, VueLoader.load(componentURL, comp.name));
					}
				}
			}
		});
