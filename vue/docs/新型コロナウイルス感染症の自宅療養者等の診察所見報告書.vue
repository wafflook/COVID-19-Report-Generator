     ...
     var doc = await PDFLib.PDFDocument.load(await fetch(`${this.$options.name}.pdf`).then(r=>r.arrayBuffer()))
         ...
         var date = moment(this.$root.共通.患者の生年月日)
          if(date.isValid()){
             ...
             page.drawText(this.$root.共通.病院の名称,{x:325,y:717,size:9})
             page.drawText(this.$root.共通.病院の名称カナ,{x:325,y:702,size:9})
             page.drawText(phone.format(this.$root.共通.病院の連絡先) || '',{x:325,y:687,size:9})
             page.drawText(this.$root.共通.医師の氏名,{x:325,y:670,size:9})
             
             page.drawText(phone.format(this.$root.共通.病院の連絡先の時間外) || '',{x:442,y:687,size:9})



             
             page.drawText(`${this.$root.共通.患者の氏名} ${this.$root.共通.患者の氏名カナ}`,{x:84,y:639,size:10})
             page.drawText(this.$root.共通.患者の性別,{x:362,y:639,size:10})
             page.drawText(phone.format(this.$root.共通.患者の住所の電話番号) || '',{x:362,y:619,size:10})


             page.drawText([this.$root.共通.患者の住所の都道府県,this.$root.共通.患者の住所の市区町村].join(''),{x:84,y:624,size:10})
             page.drawText([this.$root.共通.患者の住所の町域番地,this.$root.共通.患者の住所の建物号室].join(''),{x:84,y:610,size:10})
